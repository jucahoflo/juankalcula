...
function decodeE96(code) {
    const c = code.trim().toUpperCase();
    if (c.length !== 3) throw new Error("E96 requiere 3 caracteres");

    const num = c.substring(0, 2);
    const letra = c.substring(2);

    const base = e96Values[num];
    const mult = e96Mults[letra];

    if (!base) throw new Error("Número " + num + " no válido");
    if (!mult) throw new Error("Letra " + letra + " no válida");

    let val = base * mult;
    return val >= 1000 ? (val/1000).toFixed(2) + " kΩ" : val.toFixed(1) + " Ω";
}